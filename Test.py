...
col1.image(image, use_column_width=True, caption="灰度图")
...
col2.image(thresh, use_column_width=True, clamp=True, caption="阈值处理")

# 最小滤波处理
kernel = np.ones((3,3),np.uint8) 
# iteration的值越高，模糊程度(腐蚀程度)就越高 呈正相关关系且只能是整数
erosion = cv2.erode(thresh, kernel, iterations = 1)
st.text("{} {} {}".format(type(erosion), type(erosion[0]), type(erosion[0][0])))
col3.image(erosion, use_column_width=True, clamp=True, caption="最小滤波处理")
...
col4.image(img, use_column_width=True, clamp=True, caption="轮廓")

# 霍夫变换
blank2 = np.zeros(image.shape, np.uint8)
col5.image(img, use_column_width=True, clamp=True, caption="霍夫变换前")
lines = cv2.HoughLinesP(img, 1, np.pi/180, 100, minLineLength=200, maxLineGap=100)
for line in lines:
    x1,y1,x2,y2 = line[0]
    cv2.line(blank2,(x1,y1),(x2,y2),(0,255,0),2)
col6.image(blank2, use_column_width=True, clamp=True, caption="霍夫变换")
